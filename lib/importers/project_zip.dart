...
class ProjectZipImporter extends Importer{
  ...
  Future<bool> askUser(BuildContext context) {
    ...
    if (File(zipPath).lengthSync() < 1e9) { //1 GB limit
      okay.complete(true);
      return okay.future;
    }
  ...
  Future<Project> generateProject() async {
    final projectPath = archive.findFile("project.json");
    if (projectPath == null) {
      throw const FormatException("Cannot process archive. Missing project.json.");
    }
  ...
  Future<void> setupFiles() async{
    extractFiles(zipPath, project!.storagePath).then((_) => project!.loaded.complete());
  }
}
